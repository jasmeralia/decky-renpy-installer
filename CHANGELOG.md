...
Version headings are added when a release is cut. Because CI assigns the version
number at merge time, in-flight changes go under `## [Unreleased]`.
...
- The `version` field in `package.json` and `plugin.json` is now a `0.0.0`
  placeholder, stamped from the release tag at package time by
  `scripts/write_version.mjs`.
- `ci.yml` now runs on pull requests only; `release.yml` owns `master` and runs
  the same audit/lint/test/build gate before tagging.
- Release notes are generated by GitHub from merged PR titles.

### Added
- `pnpm test` now runs in CI (it previously ran only locally).
